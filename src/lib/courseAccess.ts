 ...
 * independent of Dog Smart Training & Behaviour's own site session
 * (each site has its own cookie, signed with its own local secret).
 *
 ...
 *
 * Required env vars:
 *   COURSE_ACCESS_TOKEN_SECRET  — shared with Base44 AND Dog Smart's site
 *                                 (verifies the handshake token Base44
 *                                 mints)
 *   LEARNING_PLATFORM_API_KEY   — shared with Base44 AND Dog Smart's site
 *                                 (authenticates our calls to
 *                                 checkCourseAccess)
 *   LEARNING_SESSION_SECRET     — local to THIS site only, signs this
 *                                 site's own session cookie. Must be a
 *                                 DIFFERENT value from Dog Smart's copy.
 */

const SESSION_COOKIE = "briarrose_learning_session";
 ...
 * here, and never trusted from anything the browser sends. In practice
 * this site only ever cares about "gundog_course", but the check returns
 * whatever Base44 says the client is entitled to.
 */
