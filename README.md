...
- **Site Settings** (singleton, at the top of the sidebar) — everything that isn't tied to a specific class, dog, or post: homepage text, contact details, social links, newsletter copy, footer text, page headers, and default SEO. Because this one document covers so much, it's split into tabs across the top of the editing screen — **General**, **Homepage: Hero**, **Homepage: Philosophy**, **Homepage: Booking Banner**, **Contact Details**, **Social Links**, **Newsletter & Footer**, **Page Headers**, and **Branding & SEO**. Your phone number and email address are under the **Contact Details** tab, along with **"Where enquiries are sent"** — the inbox that gets a copy of every Contact page submission (falls back to the Email address above if left blank); your logo is under **Branding & SEO**.
  - **Page Headers** is the one to check if you're looking for text and can't find it anywhere else: it's the small label + headline (+ sometimes a supporting line) shown at the very top of every listing page — Classes, Dogs, Testimonials, Gallery, Video Hub, Journal, FAQ, Events, Contact, plus the "Classes & Services" teaser partway down the homepage. It's split into one collapsible section per page, each titled with the exact page it controls (e.g. "Classes Page (the full /classes page)") — click a section's title to open it, edit, publish. Every field title inside also repeats the page name (e.g. "Classes page — headline"), so it's clear which page you're editing even once a section is open.
